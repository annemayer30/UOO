...
if best_result:
    ...
    st.write(f"**배터리 용량**: {best_result['batt_kWh']} kWh")
    st.write(f"**PCS 용량**: {best_result['pcs_kW']} kW")
    st.dataframe(pd.DataFrame([best_result["summary"]]))

    fig, axes = plt.subplots(3, 1, figsize=(10, 12))
    axes[0].plot(best_result["thour"], best_result["Ebatt"] / 3.6e6, label="Battery Energy [kWh]", linewidth=1.5)
    ...
    st.warning("최적화 실패 또는 유효한 결과 없음.")
